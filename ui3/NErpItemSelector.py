...
class NErpItemSelector(QDialog, Ui_Dialog):
    ...
    def setup_ui(self):
        ...
        self.doSearchPushButton.clicked.connect(self.do_research)
    ...
    def accept(self) -> None:
        ...
        if c > 0:
            d = int(c / 3)
            for i in range(d):
                first_index: QModelIndex = indexes[i * 3]
                r_index = first_index.row()
                item_erp_id = self.__model.item(r_index, 0).text()
                item_description = self.__model.item(r_index, 1).text()
                item_unit = self.__model.item(r_index, 2).text()
                self.select_items.append((item_erp_id, item_description, item_unit))
