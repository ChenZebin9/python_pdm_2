    ...
    def __add_items(self):
        try:
            if self.sender() is self.addButton:
                ...
                rsp = dlg.exec_()
                if rsp == 1:
                    parent_tag_id = 266 if self.__from_storage == 'F' else 2064
                    # 获取选择项目
                    selected_items = dlg.select_items
                    for item in selected_items:
                        one_row_in_table = [QStandardItem(self.__default_product_id)]
                        erp_id = item[0]
                        tag_2_erp_id = Tag.get_tags(self.__database, name=erp_id, parent_id=parent_tag_id)
                        part_id = ''
                        if len(tag_2_erp_id) > 0:
                            tag_2_part = tag_2_erp_id[0].tag_id
                            parts_2_tag = Part.get_parts_from_tag(self.__database, tag_2_part)
                            if len(parts_2_tag) > 0:
                                part_id = parts_2_tag[0].part_id
                        id_item = QStandardItem(part_id)
                        id_item.setData(self.__record_index, Qt.UserRole + 1)
                        self.__record_index += 1
                        one_row_in_table.append(id_item)
                        one_row_in_table.append(QStandardItem(item[0]))
                        one_row_in_table.append(QStandardItem(item[2]))
                        one_row_in_table.append(QStandardItem(item[1]))
                        one_row_in_table.append((QStandardItem('0.00')))
                        self.__table_modal.appendRow(one_row_in_table)
                    self.itemsTableView.resizeColumnsToContents()
            if len(self.__one_import_data):
                for r in self.__one_import_data:
                    ...
                    if r[1] is not None and type(r[1]) == str and len(r[1]) > 0:
                        part_id = -1
                        if type(r[1]) == int:
                            part_id = r[1]
                        elif type(r[1]) == float:
                        ...
                        if self.__from_storage != 'F':
                            erp_code = the_part.get_specified_tag(database=self.__database,
                                                                  tag_name='巨轮中德ERP物料编码')
                        else:
                            erp_code = the_part.get_specified_tag(database=self.__database,
                                                                  tag_name='巨轮智能ERP物料编码')
                        if len(erp_code) > 0:
                            ...
                            qty = '0.00'
                            if type(r[3]) == int or type(r[3]) == float:
                                qty = '{:.02f}'.format(r[3])
                            elif type(r[3]) == str:
                                qty = r[3]
                            one_row_in_table.append(QStandardItem(qty))
                        ...
                        self.__record_index += 1
                        one_row_in_table.append(id_item)
                        erp_code_item = QStandardItem(r[2])
                        one_row_in_table.append(erp_code_item)
                        if self.__from_storage != 'F':
