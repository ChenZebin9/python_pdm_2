...
class DataTransport:
    ...
    def import_data_4_parts_list(parent=None, title=None, database=None):
        try:
            ...
            rows = ('零件号', '巨轮智能ERP物料编码', '巨轮中德ERP物料编码', '外部编码', '钜欧ERP物料编码')
    ...
    @staticmethod
    def import_children_2_parts_list(parent=None, title=None, database=None):
        """
        批量导入子项目
        :param parent:
        :param title:
        ...
        try:
            ...
            if file_name[-3:].upper() == 'XLS':
                ...
                excel = ExcelHandler3(file_name)
            dialog.set_excel_mode(rows, excel, general_use=True)
            dialog.show()
        except Exception as ex:
            QMessageBox.warning(parent, '', str(ex), QMessageBox.Ok)
