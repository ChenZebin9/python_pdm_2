...
class NPartMainWindow( QMainWindow, Ui_MainWindow ):
    ...
    def __export_part_list(self):
        ...
        file_name_s = QFileDialog.getSaveFileName(parent=self, caption='保存导出文件', filter='Excel File (*.xls *.xlsx)')
        file_name = file_name_s[0]
        if file_name == '':
            QMessageBox.information(self, '', '放弃保存！', QMessageBox.Ok)
            return
        file.save(file_name)
        rsp = QMessageBox.question(self, '', '是否打开文件？', QMessageBox.Yes | QMessageBox.No)
