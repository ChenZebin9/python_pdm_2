...
class DocOutputDialog(QDialog):
    ...
    def __init__(self, parent=None, vault=None, sw_app=None, work_folder=None):
    ...
    def init_ui(self):
        ...
        h_box_1.addWidget(QLabel('3D'))
        h_box_1.addWidget(self.__3d_comb)
        ...
        self.__3d_comb.addItem('不输出')
        self.__3d_comb.addItem('转为STEP输出')
        self.__3d_comb.addItem('原类型输出')
        self.__3d_comb.setCurrentIndex(0)
        self.__3d_comb.currentIndexChanged.connect(self.__setting_change)
    ...
    def __do_output(self):
        ...
        root_folder = self.__vault.GetRootFolder() if self.__vault is not None else self.__work_folder
        if root_folder is None or len(root_folder) < 1:
            raise Exception('PDM的工作目录无效。')
        ...
        for i in range(row_count):
            ...
            for j in range(c_row_count):
                ...
                if t == 'SLDPRT' or t == 'SLDASM':
                    if self.__3d_comb.currentIndex() == 1:
                        full_name = '{0}\\{1}.{2}-{3}.STEP'.format(model_dir, id_.text(), v, name.text())
                        save2_type = 'STEP'
                    elif self.__3d_comb.currentIndex() == 2:
    ...
    def __setting_change(self, v):
        ...
        if self.sender() is self.__2d_check:
            pass
        elif self.sender() is self.__3d_comb:
            pass
        ...
        try:
            ...
            for task in self.__task_list:
                try:
                    save2_type = task[2]
                    if save2_type == 'STEP':
                        err_txt = self.__app.SaveToStep(task[4], task[3])
                    elif save2_type == 'PDF':
