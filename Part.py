...
class Part:
    ...
    def get_erp_info(self, database, is_zd=False):
        """
        获取中德ERP中德物料描述信息
        :param is_zd: True - 获取中德物料编码， False - 获取钜欧物料编码
        :param database: 数据库
        :return: 物料编码，物料描述，单位；or None
        """
        erp_code = self.get_specified_tag(database, '巨轮中德ERP物料编码' if is_zd else '钜欧ERP物料编码')
        if len(erp_code) < 1:
            return None
        return database.get_erp_info(erp_code, 0 if is_zd else 2)
