...
conn = pyodbc.connect('DRIVER={SQL Server};SERVER=192.168.100.106;UID=_user;PWD=123456;DATABASE=Greatoo_JJ_Database')
c = conn.cursor()
...
# JJPart_PartRefThumbnail
c.execute('SELECT * FROM [JJPart].[PartRefThumbnail]')
rs = c.fetchall()
for r in rs:
    t_c.execute('INSERT INTO [JJPart_PartRefThumbnail] VALUES ({0}, {1})'.format(r[0], r[1]))
print('完成 JJPart_PartRefThumbnail')
